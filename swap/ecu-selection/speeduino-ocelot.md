...
Load this file alongside `swap/ecu-selection/speeduino.md` when the installer has chosen or is evaluating the Ocelot board specifically.
For Era 1 Renix full wiring translation (Renix ECU header pins → Ocelot connector pins), see `swap/wiring/era1-renix-ocelot.md`.
...
The Ocelot includes a dedicated stepper idle control chip. This directly supports the XJ's 4-wire stepper IAC motor on all eras without an external stepper module. See `swap/ecu-selection/iac-strategy.md` for wiring details.
...
- Era 1 Renix full wiring translation → `swap/wiring/era1-renix-ocelot.md`
- Connector and pinout details → `swap/wiring/connectors/`
- Trigger configuration in TunerStudio → `swap/tuning/first-start.md`
